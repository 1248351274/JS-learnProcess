...
router.get('/', async ctx => {
    ...
    const [[{count}]] = await connection.execute('SELECT count(id) as count FROM `datas`');
    ...
    let [showDatas] = await connection.execute('SELECT * FROM `datas` limit ?,?', [start, end]);
...
router.get('/test',async ctx=>{
    ...
    ctx.render('index.html',username)
})
